...
chrome.tabs.query({ active: true, currentWindow: true }, function(tabs) {
    // Remove Wayback Machine URLs from the tab URL
    if (regexWayback.test(tabs[0].url)) {
        tabs[0].url = tabs[0].url.replace(regexWayback, '');
    }
    // Split the URL into segments and reconstruct the base URL
    const urlSegments = tabs[0].url.split('/');
    pageUrl = urlSegments.slice(0, 3).join('/');
    fullUrl = tabs[0].url;
});

// Filter GitHub user URLs from a list of social media matches
...
// Filter out only GitHub URLs from a list of social media matches
...
// Find matches in content using a regular expression
...
//List of TLDs associated with countries
const tldToCountry = [
...
// Parse the domain from a clean URL
function parseDomain(cleanURL) {
    ...
    // Sort TLDs by descending length
    tlds.sort((a, b) => b.length - a.length);

    let detectedTld = "";
    let domainWithSubdomains = "";

        // Identify the longest matching TLD
    for (let tld of tlds) {
    ...
    // Remove the TLD to get the domain and subdomains
    domainWithSubdomains = cleanURL.slice(0, -detectedTld.length);

    // Separate the main domain from the subdomains
...
function analyzeHtml(htmlContent) {
    ...
    const deducedPhoneMatches = phoneMatchesArray
    .filter(match => {
        if (match.includes('+')) {
            return false;
        }
    ...
    let linksCSV = linksToCSV(links);
    let CSVCommentsLink = commentsLink(commentsCSV, 'all-comments__' + domainName + tld.replace(/\./g,'_') + '.csv');
    let CSVLinksLink = linksLink(linksCSV, 'all-links__' + domainName + tld.replace(/\./g,'_') + '.csv');
    let combinedCSV = combinedCommentsAndLinksToCSV(comments, links);
    let CSVCombinedLink = combinedLink(combinedCSV, 'all-comments-and-links__' + domainName + tld.replace(/\./g,'_') + '.csv');
    const popupContent = document.getElementById('recon-content');
    const titleElement = document.createElement('h2');
    titleElement.textContent = "In-depth source code analysis of the page :";
    popupContent.appendChild(titleElement);
    const listElement = document.createElement('ul');
    // Link for comments
    const listItem1 = document.createElement('li');
    listItem1.appendChild(CSVCommentsLink);
    listElement.appendChild(listItem1);
    // Link for links
    const listItem2 = document.createElement('li');
    listItem2.appendChild(CSVLinksLink);
    listElement.appendChild(listItem2);
    // Link for both
    const listItem3 = document.createElement('li');
    listItem3.appendChild(CSVCombinedLink);
    listElement.appendChild(listItem3);
    
    popupContent.appendChild(listElement);
}

// Check if a string matches an HTTP endpoint pattern
...
// Display dork search results based on the match and mode
...
const fileTypePatterns = {
    ...
    'csv': { pattern: 'data:text/csv', type: 'documents', extension: '.csv' },
    'javascript': { pattern: 'data:application/javascript', type: 'code', extension: '.js' },
    'css': { pattern: 'data:text/css', type: 'code', extension: '.css' },
    'html': { pattern: 'data:text/html', type: 'code', extension: '.html' },
    'xml': { pattern: 'data:application/xml', type: 'code', extension: '.xml' },
    'text': { pattern: 'data:text/plain', type: 'documents', extension: '.txt' }
};
...
function extractComments(htmlContent) {
    // Replace newlines with a custom marker (" <-' ")
    ...
    if (matches) {
        matches.forEach(match => {
            const lineNumber = modifiedContent.substr(0, modifiedContent.indexOf(match)).split("<-'").length;
...
// Function to detect file type and extension using predefined lists
function detectFileInfo(content) {
    let fileType = '';
    let fileExtension = '';
    let encoding = '';

    // Check for specific file type patterns
    for (const [key, value] of Object.entries(fileTypePatterns)) {
        if (content.includes(value.pattern)) {
            fileType = value.type;
    ...
    // If no specific pattern found, check for file extensions in the content
    if (!fileType) {
        for (const [type, extensions] of Object.entries(fileExtensions)) {
            for (const ext of extensions) {
                const regex = new RegExp(`\\${ext}(?![a-zA-Z0-9])`);
            ...
            if (fileType) break; // Exit the loop if a file type is found
        }
    }

    // Check for encodings in the content
    ...
    return { fileType, fileExtension, encoding };
}

// Create a CSV from extracted comments
function commentsToCSV(comments) {
    const csvRows = ["Line no.,File Type,File Extension,Encoding,Comment"];
    comments.sort((a, b) => a[0] - b[0]);
    comments.forEach(comment => {
        const { fileType, fileExtension, encoding } = detectFileInfo(comment[1]);
        csvRows.push(`${comment[0]},"${fileType}","${fileExtension}","${encoding}","${comment[1].replace(/,/g, ' ')}"`);
    });
    return csvRows.join("\n");
}
...
function extractLinks (htmlContent) {
    ...
    const matches = modifiedContent.match(linkPattern);
    const links = [];
    if (matches) {
        matches.forEach(match => {
            const lineNumber = modifiedContent.substr(0, modifiedContent.indexOf(match)).split("<-'").length;
            links.push([lineNumber, match]);
        });
    }
    return links;
}

// Create a CSV from extracted links
function linksToCSV(links) {
    const csvRows = ["Line no.,File Type,File Extension,Encoding,Link"];
    links.sort((a, b) => a[0] - b[0]);
    links.forEach(link => {
        const { fileType, fileExtension, encoding } = detectFileInfo(link[1]);
        csvRows.push(`${link[0]},"${fileType}","${fileExtension}","${encoding}","${link[1].replace(/,/g, ' ')}"`);
    });
    return csvRows.join("\n");
}

// Generate a download link for the CSV file
function linksLink(csvContent, fileName) {
const blob = new Blob([csvContent], { type: 'text/csv' });
const url = URL.createObjectURL(blob);
...
link.href = url;
link.download = fileName;
link.textContent = "Download links found in CSV";
return link;
}

// Combine comments and links into a single CSV
function combinedCommentsAndLinksToCSV(comments, links) {
    const csvRows = ["Type,Line no.,File Type,File Extension,Encoding,Content"];

    comments.sort((a, b) => a[0] - b[0]);
    comments.forEach(comment => {
        const { fileType, fileExtension, encoding } = detectFileInfo(comment[1]);
        csvRows.push(`Comment,${comment[0]},"${fileType}","${fileExtension}","${encoding}","${comment[1].replace(/,/g, ' ')}"`);
    });

    links.sort((a, b) => a[0] - b[0]);
    links.forEach(link => {
        const { fileType, fileExtension, encoding } = detectFileInfo(link[1]);
        csvRows.push(`Link,${link[0]},"${fileType}","${fileExtension}","${encoding}","${link[1].replace(/,/g, ' ')}"`);
    });

    return csvRows.join("\n");
}

function combinedLink(csvContent, fileName) {
    const blob = new Blob([csvContent], { type: 'text/csv' });
    const url = URL.createObjectURL(blob);
    ...
    link.href = url;
    link.download = fileName;
    link.textContent = "Download both";
    return link;
}

/*-----------------------------------*/
/* END OF COMMENT EXTRACTION SECTION */
/*-----------------------------------*/

// Event listener for the recon checkbox
...
// Execute script to get the HTML content of the current tab
